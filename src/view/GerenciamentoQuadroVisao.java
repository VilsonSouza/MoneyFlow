...
public class GerenciamentoQuadroVisao extends JInternalFrame {
	...
	// construtor
	public GerenciamentoQuadroVisao(JDesktopPane desktop, Color backgroundTelas, MoneyFlowController controller,
			String emailUsuario) {
	...
	public void atualizaTabela() {
		...
		for (QuadroVO q : quadros)
			tableModel.addRow(new Object[] { q.getCodigo(), q.getDescricao(), q.getTotalSaida(), q.getTotalEntrada() });
	}
	...
	private void alterarQuadro() {
		AlterarQuadroVisao a = new AlterarQuadroVisao(GerenciamentoQuadroVisao.this, controller,
				quadros.get(table.getSelectedRow()));
		a.setBounds(0, 0, 350, 400);
		a.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		a.setClosable(true);
		a.getContentPane().setBackground(backgroundTelas);
	...
	private void deletarQuadro() {
		int opc = JOptionPane.showConfirmDialog(GerenciamentoQuadroVisao.this,
				"Tem certeza que deseja excluir o Quadro?\n Algumas Movimentações podem ser Excluídas", "",
		...
		if (opc == 0) {
			...
			if (controller.delQuadro(codigoQuadro)) {
				...
				JOptionPane.showMessageDialog(GerenciamentoQuadroVisao.this, "Quadro deletado com sucesso", "",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(GerenciamentoQuadroVisao.this, "Erro ao deletar Quadro", "",
